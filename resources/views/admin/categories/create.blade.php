<x-admin-layout :breadcrumbs="[
    ...
    <x-wire-card>
        <form action="{{route('admin.categories.store')}}" method="POST">
            @csrf
            <x-wire-input label="Nombre" name="nombre" placeholder="Nombre de la categoría" value="{{old('nombre')}}">
            </x-wire-input>
            <div class="mt-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Descripción</label>
                <textarea name="descripcion" rows="3" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" placeholder="Descripción de la categoría">{{old('descripcion')}}</textarea>
            </div>
            <div class="mt-4">
